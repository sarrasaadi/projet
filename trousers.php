...
if (isset($_POST['Add_to_Cart'])) {  //checking if add to cart button is clicked or not
    if (isset($_SESSION['cart'])) {    //checks if anything is in cart
        ...
        if (in_array($_POST['item_name'], $cartitems)) {   //this function checks whether the item is there in cart or not
            ...
            window.location.href='trousers.php';
            </script>";
        } else {
...
<body>
    <div class="page-content">
        <?php
        $stmt = $pdo->query("SELECT * FROM stock where item like '%trouser%' or item like '%jeans%' or product_name like '%jeans%' 
        or product_name like '%trouser%'"); //like checks whether the word jeans and trouser are in the columns item or 
        //prouduct_name of the database
        $count = 0;
